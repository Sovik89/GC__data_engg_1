...
MERGE INTO  `gcp-sovik-de-1.silver_dataset.customers` target
USING 
  (SELECT DISTINCT
    *, 
    CASE 
    ...
    True as is_active
  FROM `gcp-sovik-de-1.bronze_dataset.customers`) source
...
CREATE TABLE IF NOT EXISTS `gcp-sovik-de-1.silver_dataset.orders`
(
    order_id INT64,
    customer_id INT64,
    order_date STRING,
...
--Step 2: Update Existing Active Records if There Are Changes
MERGE INTO `gcp-sovik-de-1.silver_dataset.orders` target
USING 
  (SELECT DISTINCT
    *, 
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `gcp-sovik-de-1.bronze_dataset.orders`) source
...
CREATE TABLE IF NOT EXISTS `gcp-sovik-de-1.silver_dataset.order_items`
(
    order_item_id INT64,
    order_id INT64,
    product_id INT64,
...
--Step 2: Update Existing Active Records if There Are Changes
MERGE INTO `gcp-sovik-de-1.silver_dataset.order_items` target
USING 
  (SELECT DISTINCT
    *, 
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `gcp-sovik-de-1.bronze_dataset.order_items`) source
...
CREATE TABLE IF NOT EXISTS `gcp-sovik-de-1.silver_dataset.categories`
(
    category_id INT64,
    name STRING,
    updated_at STRING,
...
TRUNCATE TABLE `gcp-sovik-de-1.silver_dataset.categories`;

--Step 3: Insert New or Updated Records
INSERT INTO `gcp-sovik-de-1.silver_dataset.categories`
SELECT 
  *,
  CASE 
...
FROM `gcp-sovik-de-1.bronze_dataset.categories`;

--Step 1: Create the products Table in the Silver Layer
CREATE TABLE IF NOT EXISTS `gcp-sovik-de-1.silver_dataset.products`
(
  product_id INT64,
  name STRING,
  category_id INT64,
...
--Step 2: Truncate table
TRUNCATE TABLE `gcp-sovik-de-1.silver_dataset.products`;

--Step 3: Insert New or Updated Records
INSERT INTO `gcp-sovik-de-1.silver_dataset.products`
SELECT 
  *,
  CASE 
    WHEN category_id IS NULL OR name IS NULL THEN TRUE
...
FROM `gcp-sovik-de-1.bronze_dataset.products`;
-------------------------------------------------------------------------------------------------------------
--Step 1: Create the product_supplier Table in the Silver Layer
CREATE TABLE IF NOT EXISTS `gcp-sovik-de-1.silver_dataset.product_suppliers`
(
    supplier_id INT64,
    product_id INT64,
...
--Step 2: Update Existing Active Records if There Are Changes
MERGE INTO `gcp-sovik-de-1.silver_dataset.product_suppliers` target
USING 
  (SELECT 
    *, 
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active
  FROM `gcp-sovik-de-1.bronze_dataset.product_suppliers`) source
...
CREATE TABLE IF NOT EXISTS `gcp-sovik-de-1.silver_dataset.suppliers`
(
  supplier_id INT64,
...
--Step 2: Truncate table
TRUNCATE TABLE `gcp-sovik-de-1.silver_dataset.suppliers`;

--Step 3: Insert New or Updated Records
INSERT INTO `gcp-sovik-de-1.silver_dataset.suppliers`
SELECT 
  *,
  CASE 
...
FROM `gcp-sovik-de-1.bronze_dataset.suppliers`;

-------------------------------------------------------------------------------------------------------------

-- --Step 1: Create the customer_reviews Table in the Silver Layer
-- CREATE TABLE IF NOT EXISTS `gcp-sovik-de-1.silver_dataset.customer_reviews`
-- (
--     id STRING,
--     customer_id INT64,
--     product_id INT64,
...
-- --Step 2: Update Existing Active Records if There Are Changes
-- MERGE INTO `gcp-sovik-de-1.silver_dataset.customer_reviews` target
-- USING 
--   (SELECT 
--     *, 
--     CURRENT_TIMESTAMP() AS effective_start_date,
--     CURRENT_TIMESTAMP() AS effective_end_date,
--     TRUE AS is_active
--   FROM `gcp-sovik-de-1.bronze_dataset.customer_reviews`) source
...
(
  SELECT 
    -- generate deterministic id since bronze has no id
    CONCAT(customer_id, '-', product_id, '-', CAST(review_date AS STRING)) AS id,
    ...
    -- change depending on your epoch format
    TIMESTAMP_SECONDS(review_date) AS review_date,
    
    CURRENT_TIMESTAMP() AS effective_start_date,
    CURRENT_TIMESTAMP() AS effective_end_date,
    TRUE AS is_active

  FROM `gcp-sovik-de-1.bronze_dataset.customer_reviews`
) source
